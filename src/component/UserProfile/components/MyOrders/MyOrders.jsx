...
const MyOrders = ({role, userData}) => {
    ...
    useEffect(()=>{
        if(role.customer) {
            async function fetchCustomerOrders() {
                ...
                const currOrder = data.orders.filter((order)=>(order.complete===false))
                const closedOrder =  data.orders.filter((order)=>(order.complete===true))
    ...
    const innerNavItems = [
        ...
        {key:"CLOSED", value:"CLOSED", content:(<ClosedOrders closedOrders={closedOrders} closedTradieOrders={closedTradieOrders}/>)},
    ]
